...
for (const data of entry) {
  ...
  const numberRanges = value[0].split('-')
  const letter = value[1].replace(':', '')
  const password = value[2]
  if (!password.includes(letter)) continue
  const repeatedLettersInPassword = password.match(new RegExp(letter, 'gi')).length

  if (repeatedLettersInPassword > numberRanges[1] || repeatedLettersInPassword < numberRanges[0]) continue
  validPasswords++
}
console.log('answer 1:', validPasswords)
...
for (const data of entry) {
  ...
  const numberRanges = value[0].split('-')
  const letter = value[1].replace(':', '')
  const password = value[2].split('')
  if (
    (password[numberRanges[0] - 1] === letter && password[numberRanges[1] - 1] !== letter) ||
    (password[numberRanges[1] - 1] === letter && password[numberRanges[0] - 1] !== letter)
  ) {
    validPasswordsTwo++
  }
}
console.log('answer 2:', validPasswordsTwo)
